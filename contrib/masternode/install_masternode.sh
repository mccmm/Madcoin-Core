...
echo "**********************************************************************"
echo "*                                                                    *"
echo "* This script will install and configure your Madcoin masternode.  *"
echo "*                                                                    *"
...
read -p "This script will install & configure a Madcoin Masternode.  Do you wish to continue? (y/n)? " response
...
if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]
	...
	sleep 3
	cd
	git clone https://github.com/madcoinChain/Madcoin.git
	...
	# Create config for Madcoin
	echo && echo "Configuring Madcoin..."
	sleep 3
	cd ~
	cd Madcoin/src
	./madcoind -daemon
	...
	cd /root/.madcoincore 
	echo -e "rpcuser=${rpcuser}\nrpcpassword=${rpcpass}\nrpcallowip=127.0.0.1\nrpcport=11998\nrpcthreads=8\nlisten=1\nserver=1\ndaemon=1\nstaking=0\ndiscover=1\nexternalip=${ip}:3535\nmasternode=1\nmasternodeprivkey=${key}\naddnode=84.17.23.43:3535\naddnode=18.220.138.90:3535\naddnode=86.57.164.166:3535\naddnode=86.57.164.146:3535\naddnode=18.217.78.145:3535\naddnode=23.92.30.230:3535\naddnode=35.190.182.68:3535\naddnode=80.209.236.4:3535\naddnode=91.201.40.89:3535" > /root/.madcoincore/madcoin.conf
	
	# Download and install sentinel
	echo && echo "Installing Sentinel..."
	sleep 3
	cd /root/.madcoincore
	git clone https://github.com/madcoinChain/sentinelLinux.git && cd sentinelLinux
	export LC_ALL=C
	virtualenv ./venv
	./venv/bin/pip install -r requirements.txt
	echo  "* * * * * cd /root/.madcoincore/sentinelLinux && ./venv/bin/python bin/sentinel.py >/dev/null 2>&1" >> mycron
	crontab mycron
	rm mycron
	rpl dash_conf=/home/YOURUSERNAME/.madcoincore/madcoin.conf dash_conf=/root/.madcoincore/madcoin.conf sentinel.conf
	
	#Start Madcoin Daemon
	sleep 60
	cd /root/Madcoin/src
	./madcoind -daemon
	cd ~
	echo && echo && echo
	echo "Madcoin installation completed successfully.  Please wait 15 minutes and then start your masternode from your local wallet"	
else
