...
const char * const BITCOIN_CONF_FILENAME = "madcoin.conf";
const char * const BITCOIN_PID_FILENAME = "madcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "madcoin" is a composite category enabling all Madcoin-related debug output
            if(ptrCategory->count(string("madcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MadcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MadcoinCore
    // Mac: ~/Library/Application Support/MadcoinCore
    // Unix: ~/.madcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MadcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MadcoinCore";
#else
    // Unix
    return pathRet / ".madcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty madcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override madcoin.conf
